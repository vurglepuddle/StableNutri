...
just gen_l10n      # flutter gen-l10n (regenerates gitignored lib/generated/)
just ci            # install, format check, gen_l10n, build, analyze, test
```
...
`just format` deliberately targets only `./lib/core ./lib/features ./lib/l10n ./test`. `lib/generated/` is excluded
because it is gitignored generator output with its own style (see Localization) — there is nothing there to format.
...
directly. Localizations are generated separately by `just gen_l10n` (see Localization).
...
The project uses **Flutter's built-in `gen-l10n`**, configured in `l10n.yaml`. `lib/generated/` is **gitignored** and
regenerated by `just gen_l10n` (`flutter gen-l10n`) — never edit it, and never commit it. The config deliberately keeps
`output-class: S` and `output-dir: lib/generated`, so every `S.of(context).someKey` call site is unchanged from the old
`intl_utils` setup.

`S.current` and `S.load()` do **not** exist under gen-l10n. Outside a widget tree, use
`lookupS(basicLocaleListResolution([locale], S.supportedLocales))` — `lookupS` throws on an unsupported locale, so the
resolution wrapper is load-bearing, not decoration. See `main.dart`. In tests, use `l10nEn` from
`test/helpers/test_l10n.dart`.

**To add a string**, edit `lib/l10n/intl_en.arb` only, then run `just gen_l10n`.

**If the string takes arguments, you must add an `@key` block declaring its `placeholders`.** gen-l10n orders
undeclared placeholders *alphabetically* and types them all as `Object`, so a message like `"{amount} from {start} to
{end}"` silently becomes `(amount, end, start)` — it still compiles and still passes tests, and renders the arguments
...
- **TDEE** — `TDEECalc.getTDEEKcalIOM2005()` (IOM 2005, gender-specific; `caloriesProfile` covers non-binary users).
  The activity coefficient PA multiplies the weight **and** height terms — `PA x (14.2 x kg + 503 x m)`. Dropping
  those brackets is a real bug this project shipped; it cost non-sedentary users 109-489 kcal/day. It is pinned by
  `test/unit_test/tdee_iom2005_published_values_test.dart`, whose fixtures are cells of the published IOM tables
  rather than values computed from this code. See `docs/tdee-iom-2005-verification.md`.
