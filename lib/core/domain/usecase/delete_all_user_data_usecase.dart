...
class DeleteAllUserDataUsecase {
  ...
  /// is one of the *shared* fields, so [ConfigDataSource] reads it back from
  /// `appConfigBox` — the box this wipe deliberately leaves alone so theme,
  /// language and units survive. Clearing `configBox` therefore does not
  /// clear it, and `main.dart` reschedules the reminder from it on the next
  /// cold start; a cancel on its own would hold only until the user next quit
  /// the app.
  ///
