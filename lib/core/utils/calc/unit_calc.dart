...
class UnitCalc {
  ...
  /// 13.97 lb remainder clears the guard, then renders as "14", which is a
  ...
  static (int stones, double pounds) kgToStLb(
    double kg, {
    int poundsDecimals = 2,
  }) {
    final totalLbs = kg * 2.20462;
    var stones = totalLbs ~/ 14;
    var pounds = double.parse(
      (totalLbs - stones * 14).toStringAsFixed(poundsDecimals),
    );
    if (pounds >= 14.0) {
      stones += 1;
      pounds = 0.0;
