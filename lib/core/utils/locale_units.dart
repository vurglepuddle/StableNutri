...
String? countryCodeFromLocale(String localeName) {
  final beforeDot = localeName.split(RegExp(r'[.@]')).first;
  final parts = beforeDot.split(RegExp('[_-]'));
  // A script can precede the region, as in zh_Hant_TW or en_Latn_US.
  var regionIndex = 1;
  if (parts.length > 1 && RegExp(r'^[A-Za-z]{4}$').hasMatch(parts[1])) {
    regionIndex = 2;
  }
  if (parts.length <= regionIndex) return null;
  final region = parts[regionIndex];
  return RegExp(r'^[A-Za-z]{2}$').hasMatch(region)
      ? region.toUpperCase()
      : null;
}
