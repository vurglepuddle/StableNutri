...
void main() {
  test('oldest birthday uses calendar years and ignores the time of day', () {
    expect(
      ValueValidator.getFirstDate(now: DateTime(2026, 9, 22, 18)),
      DateTime(1896, 9, 22),
    );
  });

  test(
    'birthday validation accepts boundary days but rejects future dates',
    () {
      final now = DateTime(2026, 9, 22);
      expect(
        ValueValidator.isValidBirthday(DateTime(2013, 9, 22, 23), now: now),
        isTrue,
      );
      expect(
        ValueValidator.isValidBirthday(DateTime(2013, 9, 23), now: now),
        isFalse,
      );
      expect(ValueValidator.isValidBirthday(DateTime(2100), now: now), isFalse);
      expect(ValueValidator.isValidBirthday(DateTime(1800), now: now), isFalse);
    },
  );

  test('non-finite measurements never become valid values', () {
    for (final value in [
      double.nan,
      double.infinity,
      double.negativeInfinity,
    ]) {
      expect(ValueValidator.parseHeightInCm(value), isNull);
      expect(ValueValidator.parseWeightInKg(value), isNull);
      expect(ValueValidator.parseFeetInchesHeightInCm(5, value), isNull);
      expect(ValueValidator.parseStLbWeightInKg(10, value), isNull);
    }
  });
  group('ageInYears', () {
