  ...
  group('delete-all and the alarm the OS is holding', () {
    ...
    setUp(() async {
      ...
      provider = FakeHiveDBProvider(
        ...
        bodyMeasurementLogBox: await Hive.openBox<BodyMeasurementLogDBO>(
    ...
    test('the next launch cannot schedule it again', () async {
      ...
      // one of the *shared* fields, so ConfigDataSource reads it back from
      // appConfigBox — the box this wipe deliberately leaves alone. Cancelling
      // without switching it off holds only until the user next quits the app,
      // and main.dart puts the reminder straight back on the next cold start.
