...
void main() {
  group('MealEntity.fromSpFood serving label', () {
    ...
    test("FDC's raw 'undetermined' unit is presented as portion with weight "
    ...
    test('weight units are not doubled up with a gram suffix', () {
      final meal = MealEntity.fromSpFood(
        _food(servingQuantity: 100, servingUnit: 'g', servingGramWeight: 100),
      );
      expect(meal.servingSize, '100 g');
    });

    test('a portion description from the backend gets the weight appended, '
        'FDC-website style', () {
      final meal = MealEntity.fromSpFood(
        _food(
    ...
    test('a description already stating a weight is not doubled up', () {
      final meal = MealEntity.fromSpFood(
        _food(servingSize: '38 g', servingGramWeight: 38),
      );
