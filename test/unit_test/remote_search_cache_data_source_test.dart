...
void main() {
  group('RemoteSearchCacheDataSource', () {
    ...
    test('cache uses code as timestamp key when present, else name', () async {
      await ds.cache(_meal(code: 'A', name: 'Apple'));
    ...
    test(
      'cache silently skips timestamping when both code and name are null',
      () async {
    ...
    test(
      ...
      '(regression: bulk-caching must not wipe touch signal)',
      () async {
        await ds.cache(_meal(code: 'A', name: 'Apple v1'));
        ...
        expect(
          ds.getAll().firstWhere((m) => m.code == 'A').name,
          equals('Apple v2'),
    ...
    test('cacheFromSearch dedups within a single batch (later duplicate '
      ...
      final apples = ds.getAll().where((m) => m.code == 'A').toList();
    ...
    test(
      ...
      '(regression: O(N²) linear scan replaced with map dedup)',
      () async {
    ...
    group('detailed flag (thin search vs full product)', () {
      ...
      test('cacheFromSearch does not downgrade a full entry to a thin search '
          'result', () async {
      ...
      test(
        'cacheFromSearch still refreshes a thin entry with newer thin data',
        () async {
    ...
    test(
      'getAllByMostRecentlyTouched sorts entries with no timestamp last',
      () async {
        await ds.cache(_meal(code: 'A', name: 'Apple'));
        await ds.cache(_meal(code: 'B', name: 'Banana'));
