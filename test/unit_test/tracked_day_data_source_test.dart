...
void main() {
  group('TrackedDayDataSource getTrackedDaysInRange test', () {
    ...
    test('returns tracked days within range inclusively', () async {
      ...
      expect(
        result.map((e) => e.day).toList(),
        containsAll([day2, day3, day4]),
      );
    ...
    test('includes boundary days in range query', () async {
      ...
      await dataSource.saveTrackedDay(
        TrackedDayDBO(day: startDay, calorieGoal: 2000, caloriesTracked: 1500),
      );

      await dataSource.saveTrackedDay(
        TrackedDayDBO(day: endDay, calorieGoal: 2000, caloriesTracked: 1800),
      );
    ...
    test('returns empty list when no days in range', () async {
      ...
      await dataSource.saveTrackedDay(
        TrackedDayDBO(day: day1, calorieGoal: 2000, caloriesTracked: 1500),
      );
  ...
  group('TrackedDayDataSource mutations', () {
    ...
    test(
      'hasTrackedDay reports false when day is missing, true after save',
      () async {
    ...
    test(
      'addDayMacroTracked accumulates and removeDayMacroTracked subtracts',
      () async {
        await ds.saveTrackedDay(seed());

        await ds.addDayMacroTracked(
          day,
          carbsAmount: 30,
          fatAmount: 10,
          proteinAmount: 20,
    ...
    test('reconcileCaloriesAndMacrosTracked sets the totals exactly', () async {
      await ds.saveTrackedDay(seed());
    ...
    test(
      'all mutations are no-ops when the day has no tracked record',
      () async {
    ...
    test('saveTrackedDay overwrites prior entry for the same day', () async {
      await ds.saveTrackedDay(seed());
      await ds.saveTrackedDay(
        TrackedDayDBO(day: day, calorieGoal: 2500, caloriesTracked: 800),
      );
